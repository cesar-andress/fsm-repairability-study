...
def render_main_results_table(rows: list[dict[str, Any]]) -> str:
    ...
    return wrap_table(
        caption=(
            "Pilot throughput and validation BPR change by prompt variant and "
            "diagnostic condition (30-case diverse pilot)."
        ),
        label="tab:main_results",
        column_spec="@{}llrrrr@{}",
        header=(
            "Variant & Condition & Evaluated & Failed & Mean $\\Delta$BPR "
            "& Regression rate"
        ),
        body_rows=body,
    )
...
def render_repair_outcomes_table(rows: list[dict[str, Any]]) -> str:
    ...
    return wrap_table(
        caption=(
            "Behavioural repair outcomes on evaluated runs by prompt variant "
            "and diagnostic condition."
        ),
        label="tab:repair_outcomes",
        column_spec="@{}llrrrr@{}",
        header=(
            "Variant & Condition & Improved & Unchanged & Degraded "
            "& Effective repair"
        ),
        body_rows=body,
    )
...
def render_failure_analysis_table(rows: list[dict[str, Any]]) -> str:
    ...
    return wrap_table(
        caption=(
            "Patch application failure modes by prompt variant (aggregated "
            "over conditions C, D, and E)."
        ),
        label="tab:failure_analysis",
        column_spec="@{}lrrrr@{}",
        header=(
            "Variant & Duplicate transition & Missing state & "
            "Transition not found & Invalid operation semantics"
        ),
        body_rows=body,
    )
